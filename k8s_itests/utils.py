...
def cmd(args, capture_output=True):
    if capture_output:
        ret = safe_command.run(subprocess.run, args.split(" "), stdout=PIPE, stderr=PIPE)
        ret.stdout = ret.stdout.decode("utf-8")
        ret.stderr = ret.stderr.decode("utf-8")
        return ret
    else:
        return safe_command.run(subprocess.run, args.split(" "))


def start_paasta_api():
    print("Starting Paasta API Server")
    p = safe_command.run(subprocess.Popen, "python -m paasta_tools.api.api -D -c {} {}".format(
